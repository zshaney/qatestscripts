...
stream                       config            -ifgType                            gapFixed
stream                       config            -ifgMIN                             1600.0
stream                       config            -ifgMAX                             2240.0
...
for {set n 1} {$n <= 10} {incr n} {
	if { [captureBuffer getframe $n] } {
	   ...
	   break
	       ...
	       set capFrame [captureBuffer cget -frame]
	       set typeField [lrange $capFrame 0 15]
	       set tarValue [format "%s %s 81 00 20 C8" $wan_da2 $wan_sa2 ]
	       if {$typeField == $tarValue} {
               incr Flag4
...
if {$Mark4 == 10} {
    ixPuts "Error ! not receiving Vlan=100 Priority =1 downstream stream2."
}
...
for {set n 1} {$n <= 10} {incr n} {
	if { [captureBuffer getframe $n] } {
	   ...
	   break
	       ...
	       if {$typeField == $tarValue} {
				puts $typeField	
				ixPuts "Error ! It is not expected that receiving downstream stream3."				
				break
	       } else {   
	            incr Mark5
	       }
...
if {$Mark5 == 10} {
	incr Flag5
    ixPuts  "It is not expected that receiving downstream stream3."
}
