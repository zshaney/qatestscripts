...
for {set k 1} {$k <= 15} {incr k} {
	if { [captureBuffer getframe $k] } {
	   ...
	   break
	     ...
	     set capFrame [captureBuffer cget -frame]
	     set typeField [lrange $capFrame 0 15]
	     set tarValue [format "%s %s 81 00 20 C8" $lan_da3 $lan_sa3 ]
	     if {$typeField == $tarValue} {
            incr Flag3
...
for {set m 1} {$m <= 15} {incr m} {
	if { [captureBuffer getframe $m] } {
	   ...
	   break
	     ...
	     set capFrame [captureBuffer cget -frame]
	     set typeField [lrange $capFrame 0 15]
	     set tarValue [format "%s %s 81 00 40 C8" $lan_da4 $lan_sa4 ]
	     if {$typeField == $tarValue} {
            incr Flag4
...
for {set s 1} {$s <= 10} {incr s} {
	if { [captureBuffer getframe $s] } {
	   ...
	   break
	     ...
	     set capFrame [captureBuffer cget -frame]
	     set typeField [lrange $capFrame 0 15]
	     set tarValue [format "%s %s 81 00 40 C8" $wan_da2 $wan_sa2]
	     if {$typeField == $tarValue} {
            incr Flag6
		    ixPuts  "It is expected that receiving a Vlan=200 Priority =2 downstream stream1 ."
	        break
	     } else { 
		      puts $typeField
	          incr Mark6
...
if  {$Mark6 == 10} {
	ixPuts "It is expected that not receiving downstream stream2."
}
