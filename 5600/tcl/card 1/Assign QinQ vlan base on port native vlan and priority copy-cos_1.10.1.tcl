...
######### Chassis-192.168.24.194 Card-10  Port-4 #########
...
for {set i 1} {$i <= 20} {incr i} {
	if { [captureBuffer getframe $i] } {
	   ...
	   break
	       ...
	       set tarValue [format "%s %s 81 00 21 2C 81 00 20 64" $lan_da1 $lan_sa1 ]
	       if {$typeField == $tarValue	} {
              incr Flag1
	          ixPuts "It is expected that receiving a outerVlan=300&Priority =1 innerVLAN=150&priority=1 upstream stream1 ."
	          break
	          } else { 
	                puts $typeField
	                incr Mark1
...
if {$Mark1 == 20} {
    ixPuts "error ! not receiving outerVlan=300&Priority =1 innerVLAN=150&priority=1 upstream stream1"
}
...
for {set j 1} {$j <= 20} {incr j} {
	if { [captureBuffer getframe $j] } {
	    ...
	    break
	     ...
	     set tarValue [format "%s %s 81 00 40 96" $lan_da2 $lan_sa2 ]
	     if {$typeField == $tarValue} {
            incr Flag2
	        ixPuts "It is expected that receiving a  SVID=150,Spbit=2 upstream stream2 ."
	     break
	     } else { 
	           puts $typeField
	           incr Mark2
...
if {$Mark2 == 20} {
    ixPuts "error ! not receiving SVID=150,Spbit=2 upstream stream2"
}
...
for {set k 1} {$k <= 20} {incr k} {
	if { [captureBuffer getframe $k] } {
	   ...
	   break
	     ...
	     set capFrame [captureBuffer cget -frame]
	     set typeField [lrange $capFrame 0 15]
	     set tarValue [format "%s %s 81 00 61 2C" $lan_da3 $lan_sa3]
	     if {$typeField == $tarValue} {
            incr Flag2
	        ixPuts "It is expected that receiving a SVID=300,Spbit=3 upstream stream3 ."
			break
	     } else { 
	           puts $typeField
	           incr Mark2
...
if  {$Mark3 == 20} {
	ixPuts "error ! not receiving SVID=300,Spbit=3 upstream stream3"
}
...
for {set s 1} {$s <= 10} {incr s} {
	if { [captureBuffer getframe $s] } {
	   ...
	   break
		...
		set capFrame [captureBuffer cget -frame]
		set typeField [lrange $capFrame 0 15]
		set tarValue [format "%s %s 81 00 20 96" $wan_da2 $wan_sa2 ]
		if {$typeField == $tarValue} {
...
for {set s 1} {$s <= 10} {incr s} {
	if { [captureBuffer getframe $s] } {
	   ...
	   break
		...
		set tarValue [format "%s %s" $wan_da3 $wan_sa3 ]
		if {$typeField == $tarValue} {
