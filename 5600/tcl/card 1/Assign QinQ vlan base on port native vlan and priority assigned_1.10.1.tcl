...
stream                       config            -ifg                                9993920.0
stream                       config            -ifgType                            gapFixed
stream                       config            -ifgMIN                             1960.0
stream                       config            -ifgMAX                             2640.0
stream                       config            -ibg                                9993920.0
stream                       config            -enableIbg                          false
stream                       config            -isg                                9993920.0
	...
	if { [captureBuffer getframe $i] } {
	   ...
	   break
	       ...
	       if {$typeField == $tarValue	} {
              incr Flag1
	          ixPuts "It is expected that receiving a SVID=300 ,Spbits=3;CVID=100,Cpbit=1 upstream stream1 ."
	          break
	          } else { 
	                puts $typeField
	                incr Mark1
...
if {$Mark1 == 30} {
    ixPuts "error ! not receiving SVID=300 ,Spbits=3;CVID=100,Cpbit=1 upstream stream1"
}
...
#TCL API执行成功返回0 不进入if语句；执行失败返回1，进入if语句
...
for {set n 1} {$n <= 10} {incr n} {
	if { [captureBuffer getframe $n] } {
	   ...
	   break
	       ...
	       set capFrame [captureBuffer cget -frame]
	       set typeField [lrange $capFrame 0 15]
	       set tarValue [format "%s %s 81 00 40 96" $wan_da2 $wan_sa2 ]
	       if {$typeField == $tarValue} {
               incr Flag5
	           ixPuts  "It is expected that receiving a Vlan=150 Priority =2 downstream stream2 ."
	           break
	       } else { 
	              puts $typeField
	              incr Mark5
...
if {$Mark5 == 10} {
    ixPuts "Error ! not receiving Vlan=150 Priority =2 downstream stream2."
}
...
for {set n 1} {$n <= 10} {incr n} {
	if { [captureBuffer getframe $n] } {
	   ...
	   break
	       ...
	       if {$typeField == $tarValue} {
               incr Flag6
	           ixPuts  "It is expected that receiving a untag downstream stream3 ."
	           break
	       } else { 
	              puts $typeField
	              incr Mark6
...
if {$Mark6 == 10} {
    ixPuts "Error ! not receiving a untag downstream stream3."
}
