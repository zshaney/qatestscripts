...
vlan                         setDefault        
vlan                         config            -vlanID                             1000
vlan                         config            -userPriority                       4
vlan                         config            -cfi                                resetCFI
vlan                         config            -mode                               vIdle
vlan                         config            -repeat                             10
vlan                         config            -step                               1
vlan                         config            -maskval                            "1000XXXXXXXXXXXX"
vlan                         config            -protocolTagId                      vlanProtocolTag8100
vlan                         set               $chassis $card $lanport
...
stream                       config            -numBursts                          1
stream                       config            -numFrames                          3
stream                       config            -ifg                                99993920.0
stream                       config            -ifgType                            gapFixed
stream                       config            -ifgMIN                             1920.0
stream                       config            -ifgMAX                             2560.0
stream                       config            -ibg                                99993920.0
stream                       config            -enableIbg                          false
stream                       config            -isg                                99993920.0
stream                       config            -enableIsg                          false
stream                       config            -gapUnit                            gapNanoSeconds
stream                       config            -percentPacketRate                  0.0704
stream                       config            -fpsRate                            10.0
stream                       config            -bpsRate                            5440.0
stream                       config            -rateMode                           usePercentRate
stream                       config            -preambleSize                       8
...
if { [captureBuffer get $chassis $card $wanport 1 30] } {
...
for {set i 1} {$i <= 30} {incr i} {
	if { [captureBuffer getframe $i] } {
	   ...
	   break
	       ...
	       set tarValue [format "%s %s 81 00 23 E8" $lan_da1 $lan_sa1 ]
	       if {$typeField == $tarValue	} {
              incr Flag1
	          ixPuts "It is expected that receiving a CVID1=1000,Cpbits1=1 upstream stream1 ."
	          break
	          } else { 
	                puts $typeField
	                incr Mark1
...
if {$Mark1 == 30} {
    ixPuts "error ! not receiving CVID1=1000,Cpbits1=1 upstream stream1"
}
...
for {set k 1} {$k <= 30} {incr k} {
	if { [captureBuffer getframe $k] } {
	   ...
	   break
	     ...
	     if {$typeField == $tarValue} {
	        incr Flag2
			ixPuts "It is expected that receiving CVID1=1000,Cpbits1=2 upstream stream2."
	        break
	     } else { 
		      puts $typeField 
	           incr Mark2
...
if  {$Mark2 == 30} {
    ixPuts "error ! receiving upstream CVID1=1000,Cpbits1=2 stream2."
}
...
for {set j 1} {$j <= 30} {incr j} {
	if { [captureBuffer getframe $j] } {
	    ...
	    break
	     ...
	     set capFrame [captureBuffer cget -frame]
	     set typeField [lrange $capFrame 0 15]
	     set tarValue [format "%s %s 81 00 63 E8" $lan_da3 $lan_sa3 ]
	     if {$typeField == $tarValue} {
            incr Flag3
	        ixPuts "It is expected that receiving a CVID1=1000,Cpbits1=3 upstream stream3."
	     break
	     } else { 
	           puts $typeField
	           incr Mark3
...
if {$Mark3 == 30} {
    ixPuts "error ! not receiving CVID1=1000,Cpbits1=3 upstream stream3"
}
...
for {set m 1} {$m <= 30} {incr m} {
	if { [captureBuffer getframe $m] } {
	   ...
	   break
	     ...
	     set capFrame [captureBuffer cget -frame]
	     set typeField [lrange $capFrame 0 15]
	     set tarValue [format "%s %s 81 00 83 E8" $lan_da4 $lan_sa4]
	     if {$typeField == $tarValue} {
		 incr Flag4
	        ixPuts "It is expected that receiving CVID1=1000,Cpbits1=4 upstream stream4."
	        break
	     } else { 
		      puts $typeField
	          incr Mark4
...
if  {$Mark4 == 30} {
    ixPuts "error ! receiving upstream CVID1=1000,Cpbits1=4 stream4."
}
...
for {set r 1} {$r <= 10} {incr r} {
	if { [captureBuffer getframe $r] } {
	   ...
	   break
		...
		set capFrame [captureBuffer cget -frame]
		set typeField [lrange $capFrame 0 15]
		set tarValue [format "%s %s 81 00 23 E8" $wan_da1 $wan_sa1 ]
		if {$typeField == $tarValue} {
		   incr Flag5
		   ixPuts  "It is expected that receiving a CVID1=1000,Cpbits1=1 downstream stream1 ."
		   break
		} else { 
		     puts $typeField
		     incr Mark5
...
if {$Mark5 == 10} {
    ixPuts "error ! not receiving  CVID1=1000,Cpbits1=1 downstream stream1"
}
...
for {set s 1} {$s <= 10} {incr s} {
	if { [captureBuffer getframe $s] } {
	   ...
	   break
	     ...
	     set capFrame [captureBuffer cget -frame]
	     set typeField [lrange $capFrame 0 15]
	     set tarValue [format "%s %s 81 00 43 E8" $wan_da2 $wan_sa2]
	     if {$typeField == $tarValue} {
		 incr Flag6
	        ixPuts "It is expected that receiving CVID1=1000,Cpbits1=2 downstream stream2."
	        break
	     } else { 
			  puts $typeField
	          incr Mark6
...
if  {$Mark6 == 10} {
    ixPuts "error ! receiving downstream CVID1=1000,Cpbits1=2 stream2."
}

set Flag [expr $Flag1 + $Flag3 + $Flag5]
set errFlag [expr $Flag2 + $Flag4 + $Flag6]
if {$errFlag == 3} {
	if {$Flag == 3} {
		ixPuts "#########################  result  #####################"
		puts "this case pass!"
