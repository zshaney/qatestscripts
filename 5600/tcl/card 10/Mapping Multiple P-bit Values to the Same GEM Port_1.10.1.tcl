...
for {set j 1} {$j <= 30} {incr j} {
	if { [captureBuffer getframe $j] } {
	   ...
	   break
	       ...
	       set capFrame [captureBuffer cget -frame]
	       set typeField [lrange $capFrame 0 15]
	       set tarValue [format "%s %s 81 00 40 64" $lan_da2 $lan_sa2 ]
	       if {$typeField == $tarValue	} {
              incr Flag2
	          ixPuts "It is expected that receiving a Vlan=100 Priority =2 upstream stream2 ."
	          break
	          } else { 
	                puts $typeField
	                incr Mark2
...
if {$Mark2 == 30} {
    ixPuts "error ! not receiving Vlan=100 Priority =2 upstream stream2"
}
...
for {set n 1} {$n <= 30} {incr n} {
	if { [captureBuffer getframe $n] } {
	    ...
	    break
	     ...
	     set capFrame [captureBuffer cget -frame]
	     set typeField [lrange $capFrame 0 15]
	     set tarValue [format "%s %s 81 00 20 C8" $lan_da4 $lan_sa4 ]
	     if {$typeField == $tarValue} {
            incr Flag4
	        ixPuts "It is expected that receiving a Vlan=200 Priority =1 upstream stream4."
	     break
	     } else { 
	           puts $typeField
	           incr Mark4
...
if {$Mark4 == 30} {
    ixPuts "error ! not receiving Vlan=200 Priority =1 upstream stream4"
}
...
for {set p 1} {$p <= 30} {incr p} {
	if { [captureBuffer getframe $p] } {
	    ...
	    break
	     ...
	     set capFrame [captureBuffer cget -frame]
	     set typeField [lrange $capFrame 0 15]
	     set tarValue [format "%s %s 81 00 40 C8" $lan_da5 $lan_sa5 ]
	     if {$typeField == $tarValue} {
            incr Flag5
	        ixPuts "It is expected that receiving a Vlan=200 Priority =2 upstream stream5."
	     break
	     } else { 
	           puts $typeField
	           incr Mark5
...
if {$Mark5 == 30} {
    ixPuts "error ! not receiving Vlan=200 Priority =2 upstream stream5"
}
...
for {set q 1} {$q <= 30} {incr q} {
	if { [captureBuffer getframe $q] } {
	    ...
	    break
	     ...
	     set capFrame [captureBuffer cget -frame]
	     set typeField [lrange $capFrame 0 11]
	     set tarValue [format "%s %s" $lan_da6 $lan_sa6 ]
	     if {$typeField == $tarValue} {
            ixPuts "error ! receiving upstream stream6."
			puts $typeField
	        break
	     } else { 
	           incr Mark6
...
if {$Mark6 == 30} {
    incr Flag6
	ixPuts "It is expected that not receiving upstream stream6"
}
...
for {set s 1} {$s <= 10} {incr s} {
	if { [captureBuffer getframe $s] } {
	   ...
	   break
		...
		set capFrame [captureBuffer cget -frame]
		set typeField [lrange $capFrame 0 15]
		set tarValue [format "%s %s 81 00 00 C8" $wan_da2 $wan_sa2 ]
		if {$typeField == $tarValue} {
		   incr Flag8
		   ixPuts  "It is expected that receiving a Vlan=200 Priority =0 downstream stream2 ."
		   break
		} else { 
		     puts $typeField
		     incr Mark8
...
if {$Mark8 == 10} {
    ixPuts "error ! not receiving  Vlan=200 Priority =0 downstream stream2"
}

set Flag [expr $Flag1 + $Flag2 + $Flag4 + $Flag5 + $Flag7 + $Flag8]
set errFlag [expr $Flag3 + $Flag6]
if {$errFlag == 2} {
	if {$Flag == 6} {
		ixPuts "#########################  result  #####################"
		puts "this case pass!"
