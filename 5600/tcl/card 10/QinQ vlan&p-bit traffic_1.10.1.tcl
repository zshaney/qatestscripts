...
stream                       config            -percentPacketRate                  0.00704
	...
	if { [captureBuffer getframe $i] } {
	   ...
	   break
	       ...
	       if {$typeField == $tarValue	} {
              ixPuts "error ! not receiving CVID1=100,Cpbits1=1 upstream stream1"
	          puts $typeField
	          break
	          } else { 
	                incr Mark1
...
if {$Mark1 == 30} {
    incr Flag1
    ixPuts "It is expected that not receiving a CVID1=100,Cpbits1=1 upstream stream1 ."
}
...
for {set j 1} {$j <= 30} {incr j} {
	if { [captureBuffer getframe $j] } {
	    ...
	    break
	     ...
	     if {$typeField == $tarValue} {
            puts $typeField
	        ixPuts "error ! not receiving CVID1=100,Cpbits2=2 upstream stream2"
	        break
	     } else {       
	           incr Mark2
	     }
...
if {$Mark2 == 30} {
    incr Flag2
	 ixPuts "It is expected that not receiving a CVID1=100,Cpbits2=2 upstream stream2 ."
...
for {set k 1} {$k <= 30} {incr k} {
	if { [captureBuffer getframe $k] } {
	   ...
	   break
	     ...
	     set tarValue [format "%s %s 81 00 41 2C 81 00 20 96" $lan_da3 $lan_sa3]
	     if {$typeField == $tarValue} {
            incr Flag3
	        ixPuts "It is expected that receiving a SVID=300 Spbit=2 ; CVID=150 Cpbit=1 upstream stream3 ."
	        break
	     } else { 
		    puts $typeField
	        incr Mark3
...
if  {$Mark3 == 30} {
    ixPuts "error ! not receiving SVID=300 Spbit=2 ; CVID=150 Cpbit=1 upstream stream3."
}
...
for {set m 1} {$m <= 30} {incr m} {
	if { [captureBuffer getframe $m] } {
	   ...
	   break
	     ...
	     set capFrame [captureBuffer cget -frame]
	     set typeField [lrange $capFrame 0 11]
	     set tarValue [format "%s %s" $lan_da4 $lan_sa4]
	     if {$typeField == $tarValue} {
            ixPuts "error ! not receiving CVID2=150,Cpbits2=2 upstream stream4."
	        puts $typeField
	        break
	     } else { 
			 incr Mark4
...
if  {$Mark4 == 30} {
    incr Flag4
	ixPuts "It is expected that not receiving a CVID2=150,Cpbits2=2 upstream stream4 ."
}
...
for {set i 1} {$i <= 30} {incr i} {
	if { [captureBuffer getframe $i] } {
	   ...
	   break
	       ...
	       set tarValue [format "%s %s 81 00 20 96" $wan_da1 $wan_sa1 ]
	       if {$typeField == $tarValue	} {
              incr Flag5
	          ixPuts "It is expected that receiving a CVID=150 Cpbit=1 downstream stream1 ."
	          break
	       } else { 
	                puts $typeField
	                incr Mark5
...
if {$Mark5 == 30} {
    ixPuts "error ! not receiving CVID=150 Cpbit=1 downstream stream1"
}
