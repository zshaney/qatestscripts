...
set da1 "00 09 0A 00 01 01"
set sa1 "00 09 0A 00 02 01"
set da2 "00 09 0A 00 01 02"
set sa2 "00 09 0A 00 02 02"
set da3 "00 09 0A 00 01 03"
set sa3 "00 09 0A 00 02 03"
set da4 "00 09 0A 00 01 04"
set sa4 "00 09 0A 00 02 04"
...
stream                       config            -rateMode                           usePercentRate
stream                       config            -preambleSize                       8
stream                       config            -sa                                 $da4
stream                       config            -saRepeatCounter                    idle
stream                       config            -saStep                             1
stream                       config            -saMaskValue                        "00 00 00 00 00 00"
stream                       config            -saMaskSelect                       "00 00 00 00 00 00"
stream                       config            -enableSaContinueFromLastValue      false
stream                       config            -da                                 $sa4
stream                       config            -daRepeatCounter                    idle
stream                       config            -daStep                             1
stream                       config            -daMaskValue                        "00 00 00 00 00 00"
stream                       config            -daMaskSelect                       "00 00 00 00 00 00"
...
for {set i 1} {$i <= 10} {incr i} {
	if { [captureBuffer getframe $i] } {
	   ...
	   break
	       ...
	       set tarValue [format "%s %s 81 00 20 64" $da1 $sa1 ]
...
for {set j 1} {$j <= 10} {incr j} {
	if { [captureBuffer getframe $j] } {
	   ...
	   break
	       ...
	       set capFrame [captureBuffer cget -frame]
	       set typeField [lrange $capFrame 0 15]
	       set tarValue [format "%s %s 81 00 40 64" $da2 $sa2 ]
	       if {$typeField == $tarValue	} {
              incr Flag2
...
for {set m 1} {$m <= 10} {incr m} {
	if { [captureBuffer getframe $m] } {
	    ...
	    break
	     ...
	     set capFrame [captureBuffer cget -frame]
	     set typeField [lrange $capFrame 0 15]
	     set tarValue [format "%s %s 81 00 20 C8" $da3 $sa3 ]
	     if {$typeField == $tarValue} {
            incr Flag3
	        ixPuts "It is expected that receiving a Vlan=200 Priority =1 upstream stream3."
	     break
	     } else { 
	           puts $typeField
	           incr Mark3
...
if {$Mark3 == 10} {
    ixPuts "error ! not receiving Vlan=200 Priority =1 upstream stream3"
}
...
for {set n 1} {$n <= 10} {incr n} {
	if { [captureBuffer getframe $n] } {
	    ...
	    break
	     ...
	     set capFrame [captureBuffer cget -frame]
	     set typeField [lrange $capFrame 0 15]
	     set tarValue [format "%s %s 81 00 40 C8" $da4 $sa4 ]
	     if {$typeField == $tarValue} {
            incr Flag4
	        ixPuts "It is expected that receiving a Vlan=200 Priority =2 upstream stream4."
	     break
	     } else { 
	           puts $typeField
	           incr Mark4
...
if {$Mark4 == 10} {
    ixPuts "error ! not receiving Vlan=200 Priority =2 upstream stream4"
}
...
for {set s 1} {$s <= 10} {incr s} {
	if { [captureBuffer getframe $s] } {
	   ...
	   break
		...
		set capFrame [captureBuffer cget -frame]
		set typeField [lrange $capFrame 0 15]
		set tarValue [format "%s %s 81 00 40 C8" $sa4 $da4 ]
		if {$typeField == $tarValue} {
		   incr Flag6
		   ixPuts  "It is expected that receiving a Vlan=200 Priority =2 downstream stream4 ."
		   break
		} else { 
		     puts $typeField
		     incr Mark6
...
if {$Mark6 == 10} {
    ixPuts "error ! not receiving  Vlan=200 Priority =2 downstream stream4"
}
