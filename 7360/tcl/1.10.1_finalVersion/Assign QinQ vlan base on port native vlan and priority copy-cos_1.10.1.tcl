...
set da1 "00 09 0E 00 01 01"
set sa1 "00 09 0E 00 02 01"
set da2 "00 09 0E 00 01 02"
set sa2 "00 09 0E 00 02 02"
set da3 "00 09 0E 00 01 03"
set sa3 "00 09 0E 00 02 03"
set da4 "00 09 0E 00 01 04"
set sa4 "00 09 0E 00 02 04"
...
stream                       config            -rateMode                           usePercentRate
stream                       config            -preambleSize                       8
stream                       config            -sa                                 $sa3
stream                       config            -saRepeatCounter                    idle
stream                       config            -saStep                             1
stream                       config            -saMaskValue                        "00 00 00 00 00 00"
stream                       config            -saMaskSelect                       "00 00 00 00 00 00"
stream                       config            -enableSaContinueFromLastValue      false
stream                       config            -da                                 $da3
stream                       config            -daRepeatCounter                    idle
stream                       config            -daStep                             1
stream                       config            -daMaskValue                        "00 00 00 00 00 00"
stream                       config            -daMaskSelect                       "00 00 00 00 00 00"
...
stream                       config            -rateMode                           usePercentRate
stream                       config            -preambleSize                       8
stream                       config            -sa                                 $da3
stream                       config            -saRepeatCounter                    idle
stream                       config            -saStep                             1
stream                       config            -saMaskValue                        "00 00 00 00 00 00"
stream                       config            -saMaskSelect                       "00 00 00 00 00 00"
stream                       config            -enableSaContinueFromLastValue      false
stream                       config            -da                                 $sa3
stream                       config            -daRepeatCounter                    idle
stream                       config            -daStep                             1
stream                       config            -daMaskValue                        "00 00 00 00 00 00"
stream                       config            -daMaskSelect                       "00 00 00 00 00 00"
...
for {set i 1} {$i <= 10} {incr i} {
	if { [captureBuffer getframe $i] } {
	   ...
	   break
	       ...
	       set tarValue [format "%s %s 81 00 21 2C 81 00 20 64" $da1 $sa1 ]
	       if {$typeField == $tarValue	} {
              incr Flag1
	          ixPuts "It is expected that receiving a outerVlan=300&Priority=1 innerVLan=100&priority=1 upstream stream1 ."
	          break
	          } else { 
	                puts $typeField
	                incr Mark1
...
if {$Mark1 == 10} {
    ixPuts "error ! not receiving outerVlan=300&Priority=1 innerVLan=100&priority=1 upstream stream1"
}
...
for {set j 1} {$j <= 10} {incr j} {
	if { [captureBuffer getframe $j] } {
	    ...
	    break
	     ...
	     set tarValue [format "%s %s 81 00 40 96" $da2 $sa2 ]
	     if {$typeField == $tarValue} {
            ixPuts "It is expected that receiving a Vlan=150 Priority=2 upstream stream2."
			incr Flag2
	        break
	     } else { 
	           puts $typeField
...
if {$Mark2 == 10} {
	...
	ixPuts "error ! not receiving Vlan=150 Priority=2 upstream stream2"
}
...
for {set k 1} {$k <= 10} {incr k} {
	if { [captureBuffer getframe $k] } {
	    ...
	    break
	     ...
	     set capFrame [captureBuffer cget -frame]
	     set typeField [lrange $capFrame 0 15]
	     set tarValue [format "%s %s 81 00 61 2C" $da3 $sa3]
	     if {$typeField == $tarValue} {
            ixPuts "It is expected that receiving a Vlan=300 Priority=3 upstream stream3."
	        incr Flag3
	        break
	     } else { 
	           puts $typeField
...
if  {$Mark3 == 10} {
    ...
    ixPuts "error ! not receiving Vlan=300 Priority=3 upstream stream3."
}
...
for {set r 1} {$r <= 10} {incr r} {
	if { [captureBuffer getframe $r] } {
	   ...
	   break
		...
		set capFrame [captureBuffer cget -frame]
		set typeField [lrange $capFrame 0 15]
		set tarValue [format "%s %s 81 00 20 64" $sa1 $da1 ]
		if {$typeField == $tarValue} {
		   incr Flag4
...
for {set s 1} {$s <= 10} {incr s} {
	if { [captureBuffer getframe $s] } {
	   ixPuts "Error ! failed to get captureBuffer on $chassis $card $lanport"	
	   break
	} else {
		set capFrame [captureBuffer cget -frame]
		set typeField [lrange $capFrame 0 15]
		set tarValue [format "%s %s 81 00 20 96" $sa2 $da2 ]
		if {$typeField == $tarValue} {
		   incr Flag5
		   ixPuts  "It is expected that receiving a Vlan=150 Priority =1 downstream stream2 ."
		   break
		} else { 
		     puts $typeField
		     incr Mark5
...
if {$Mark5 == 10} {
    ixPuts "error ! not receiving  Vlan=150 Priority =1 downstream stream2"
}

set Flag6 0
set Mark6 0
for {set t 1} {$t <= 10} {incr t} {
	if { [captureBuffer getframe $t] } {
	   ixPuts "Error ! failed to get captureBuffer on $chassis $card $lanport"	
	   break
	} else {
		set capFrame [captureBuffer cget -frame]
		...
		set tarValue1 [format "%s %s" $sa3 $da3]
		set tarValue2 [format "81 00"]
		if {$typeField1 == $tarValue1} {
		   if {$typeField2 != $tarValue2} {
		      incr Flag6
		      ixPuts  "It is expected that receiving a untagged downstream stream3 ."
		      break
		   } else {
		          puts $typeField
				  incr Mark6
...
if {$Mark6 == 10} {
    ixPuts "error ! not receiving  untagged downstream stream3"
}

set Flag [expr $Flag1 + $Flag2 + $Flag3 + $Flag4 + $Flag5 + $Flag6]
if {$Flag == 6} {
	ixPuts "#########################  result  #####################"
	puts "this case pass!"
	ixPuts "########################################################"
