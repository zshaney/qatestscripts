...
set da1 "00 09 0C 00 01 01"
set sa1 "00 09 0C 00 02 01"
set da2 "00 09 0C 00 01 02"
set sa2 "00 09 0C 00 02 02"
set da3 "00 09 0C 00 01 03"
set sa3 "00 09 0C 00 02 03"
set da4 "00 09 0C 00 01 04"
set sa4 "00 09 0C 00 02 04"
...
stream                       config            -rateMode                           usePercentRate
stream                       config            -preambleSize                       8
stream                       config            -sa                                 $sa4
stream                       config            -saRepeatCounter                    idle
stream                       config            -saStep                             1
stream                       config            -saMaskValue                        "00 00 00 00 00 00"
stream                       config            -saMaskSelect                       "00 00 00 00 00 00"
stream                       config            -enableSaContinueFromLastValue      false
stream                       config            -da                                 $da4
stream                       config            -daRepeatCounter                    idle
stream                       config            -daStep                             1
stream                       config            -daMaskValue                        "00 00 00 00 00 00"
stream                       config            -daMaskSelect                       "00 00 00 00 00 00"
...
for {set i 1} {$i <= 10} {incr i} {
	if { [captureBuffer getframe $i] } {
	   ...
	   break
	       ...
	       set tarValue [format "%s %s 81 00 21 2C 81 00 20 96" $da3 $sa3 ]
	       if {$typeField == $tarValue	} {
              incr Flag1
	          ixPuts "It is expected that receiving a outerVlan=300&Priority =1 innerVLAN=150&priority=1 upstream stream3 ."
	          break
	          } else { 
	                puts $typeField
	                incr Mark1
...
if {$Mark1 == 10} {
    ixPuts "error ! not receiving outerVlan=300&Priority =1 innerVLAN=150&priority=1 upstream stream3"
}
...
for {set j 1} {$j <= 10} {incr j} {
	if { [captureBuffer getframe $j] } {
	    ...
	    break
	     ...
	     set capFrame [captureBuffer cget -frame]
	     set typeField [lrange $capFrame 0 19]
	     set tarValue [format "%s %s 81 00 41 2C 81 00 40 96" $da4 $sa4 ]
	     if {$typeField == $tarValue} {
            incr Flag2
...
for {set k 1} {$k <= 10} {incr k} {
	if { [captureBuffer getframe $k] } {
	   ...
	   break
	     ...
	     set tarValue [format "%s %s" $da1 $sa1]
	     if {$typeField == $tarValue} {
...
for {set m 1} {$m <= 10} {incr m} {
	if { [captureBuffer getframe $m] } {
	   ...
	   break
	     ...
	     set capFrame [captureBuffer cget -frame]
	     set typeField [lrange $capFrame 0 11]
	     set tarValue [format "%s %s" $da2 $sa2]
	     if {$typeField == $tarValue} {
...
for {set r 1} {$r <= 10} {incr r} {
	if { [captureBuffer getframe $r] } {
	   ...
	   break
		...
		set capFrame [captureBuffer cget -frame]
		set typeField [lrange $capFrame 0 11]
		set tarValue [format "%s %s" $sa1 $da1 ]
		if {$typeField == $tarValue} {
		   ixPuts  "error !  receiving  downstream stream1."
		   puts $typeField
		   break
		} else { 
		     incr Mark5
...
if {$Mark5 == 10} {
    incr Flag5
    ixPuts "It is expected that not receiving downstream stream1 "
}
...
for {set s 1} {$s <= 10} {incr s} {
	if { [captureBuffer getframe $s] } {
	   ...
	   break
		...
		set tarValue [format "%s %s 81 00 20 96" $sa3 $da3 ]
		if {$typeField == $tarValue} {
		   ixPuts  "It is expected that receiving a Vlan=150 Priority=1 downstream stream3."
		   incr Flag6
		   break
		} else { 
		     puts $typeField
...
if {$Mark6 == 10} {
    ixPuts "error ! not receiving Vlan=150 Priority =1 downstream stream3 "
}
